...
router.get("/:id/history", async (req, res: Response): Promise<void> => {
  try {
    const userId = req.params.id;
    const r = await pool.query(
      `SELECT p.id, p.address_text, p.pickup_status, p.total_weight_kg, p.estimated_amount,
    ...
    const userRow = (await pool.query("SELECT name, phone FROM users WHERE id = $1", [userId])).rows[0];
    if (!userRow) {
